...
from common import PROJECT_ROOT, sha256
    ...
    *,
    ...
    for asset_id in asset_ids:
        ...
        resolved.append({
            "asset_id": asset_id,
            "source": source,
            "source_path": _project_relative(source, project_root),
            "source_sha256": actual_sha256,
            "size_bytes": actual_size,
            "upload_filename": _safe_asset_filename(asset_id, source),
        })
...
def _write_asset_requirement(
    ...
    for issue in issues:
        ...
        lines.append(
            f"- `{issue['asset_id']}` — {issue['reason']}{path_note}"
        )
    lines.extend([
        "",
